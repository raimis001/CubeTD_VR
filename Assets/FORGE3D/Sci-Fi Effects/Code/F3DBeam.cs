...
namespace Forge3D
{
	...
	public class F3DBeam : F3DSprite
	{
		...
		void Raycast()
		{
			...
			if (Physics.Raycast(transform.position, transform.forward, out hitPoint, MaxBeamLength, layerMask))
			{
				...
				if (rayImpact) rayImpact.position = hitPoint.point - transform.forward * 0.5f;
				if (ImpactAction != null)
				{
					RaycastBullet bullet = new RaycastBullet(hitPoint) { forward = transform.forward };
